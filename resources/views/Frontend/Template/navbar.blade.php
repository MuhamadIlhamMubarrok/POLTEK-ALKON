    ...
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <div class="hidden w-full md:block md:w-auto" id="navbar-dropdown">
            <ul
                ...
                hover:bg-gradient-to-r from-[#129A6A] to-[#A5BBB6] md:hover:bg-transparent 
                md:border-0 md:p-0 md:w-auto text-white  
                {{ $isActive ? 'bg-gradient-to-r from-[#129A6A] to-[#A5BBB6] md:px-2 text-white hover:bg-gradient-to-br' : '' }}">
                                {{ $menu->menu }}
                                <svg class="w-2.5 h-2.5 ms-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                    fill="#129A6A" viewBox="0 0 10 6">
                            ...
                            <div id="dropdownNavbar-{{ $menu->menu_id }}"
                                ...
                                <ul class="py-2 text-sm text-gray-700 dark:text-white">
                                    @foreach ($subItems as $submenu)
                                        ...
                                        <li>
                                            <a href="{{ $subUrl }}"
                                                class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-primary dark:hover:text-white font-poppins {{ $isSubActive ? 'bg-gradient-to-r from-[#129A6A] to-[#A5BBB6] text-white' : '' }}">
                                                {{ $submenu->menu }}
                                            </a>
                                        </li>
                                    @endforeach
                        ...
                        @else
                            ...
                            <a href="{{ $url }}"
                                class="block font-poppins py-2 px-3 text-gray-900 rounded hover:bg-gradient-to-r from-[#129A6A] to-[#A5BBB6] md:hover:rounded-full md:border-0  md:p-0 text-white dark:hover:bg-gray-700  md:dark:hover:bg-transparent {{ $isActive ? 'bg-gradient-to-r from-[#129A6A] to-[#A5BBB6] hover:bg-gradient-to-br md:px-2 md:rounded-full' : '' }}">
                                {{ $menu->menu }}
                            </a>
                        @endif
                    </li>
        ...
        {{ $isContactActive ? 'bg-gradient-to-br from-[#129A6A] to-[#A5BBB6]' : 'bg-gradient-to-r from-[#129A6A] to-[#A5BBB6] hover:bg-gradient-to-br' }}">
